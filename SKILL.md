...
# SlideFlow Skill 核心指令

SlideFlow 是下一代基于 Web 的演示文稿生成框架。当你接收到用户制作 PPT 的请求时，必须严格遵守本套指令，利用灵活的“原子化组件”而非死板模版进行内容排版。
...
- 将大纲生成为 markdown 文件给用户确认，或者直接在对话中列出大纲。
- 务必向用户确认：需要生成几页？每页的重点是什么？
